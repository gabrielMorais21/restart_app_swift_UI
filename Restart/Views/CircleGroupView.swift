...
//  Restart
//
//  Created by Gabriel Morais Dias on 11/07/23.
//
...
struct CircleGroupView: View {
    ...
    var body: some View {
        ...
        }.blur(radius: isAnimating ? 0 : 10).opacity(isAnimating ? 1 : 0).scaleEffect(isAnimating ? 1 : 0.5).animation(.easeOut(duration: 1), value: isAnimating).onAppear(perform: {
            isAnimating = true
        })
    }
}
